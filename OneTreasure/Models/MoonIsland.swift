...
class MoonIsland: BaseIsland {
    ...
    init(
        ...
        super.init(id: id, name: name, descriptionText: descriptionText, isUnlocked: isUnlocked, unlocksIslandId: unlocksIslandId, islandType: .moonSoundQuest, awardsFragmentOrder: awardsFragmentOrder, islandThemeModelName: islandThemeModelName, islandIcon: islandIcon)
    }
    ...
    override func prepareExperienceViewModel(gameViewModel: GameViewModel) -> (any IslandViewModelInterface)? {
        return MoonIslandViewModel(islandData: self, gameViewModel: gameViewModel)
    }
}
