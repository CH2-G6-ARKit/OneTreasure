...
@MainActor
class VolcanoIslandViewModel: IslandViewModelInterface {
    @Published var islandData: VolcanoIsland
    @Published var currentExperienceState: VolcanoIslandExperienceState = .initializing
    ...
    enum VolcanoIslandExperienceState {
        case initializing
    ...
    required init(islandData: BaseIsland, gameViewModel: GameViewModel) {
        guard let volcanoIslandData = islandData as? VolcanoIsland else {
            fatalError("Incorrect islandData type passed to VolcanoIslandViewModel. Expected VolcanoIsland, got \(type(of: islandData)).")
        }
        self.islandData = volcanoIslandData
        self.gameViewModel = gameViewModel
        print("VolcanoIslandVireModel initialized for: \(volcanoIslandData.name)")
    }
    ...
    func startExperience(arView: ARView) {
        ...
        print("VolcanoIslandViewModel: startExperience called. State: \(currentExperienceState). Waiting for AR setup and chest world position.")
    }
    ...
    private func presentRiddle() {
        guard let gameVM = self.gameViewModel,
            ...
            isChestVisibleAndInteractive = true
            currentExperienceState = .chestFound
            print("VolcanoIslandViewModel Error: Riddle with ID \(islandData.chestRiddleId) not found for \(islandData.name).")
            return
        }
